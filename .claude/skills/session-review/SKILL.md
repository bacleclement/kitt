...
### Step 0: Delegate narrative extraction to `session-summarize`

Before computing any metrics, ensure the workspace has a fresh `{key}-summary.md`.

```
1. Resolve target summary path: {workspace-path}/{key}-summary.md
2. Check freshness:
   - If {key}-summary.md exists AND its frontmatter session_log_mtime >=
     session-log.jsonl's current mtime → summary is fresh, read it directly
   - Otherwise → invoke session-summarize on this workspace to regenerate it
3. Read the resulting {key}-summary.md for narrative sections (overview,
   timeline, feedback log, friction, smooth flows, revisions, captured rules)
```

**Why this step exists:** narrative extraction (what happened, in what order, what felt smooth, what felt rough) is LLM work that is expensive to redo on every review. Caching it as `{key}-summary.md` means review runs become cheaper and cross-workspace aggregation (`session-aggregate`, #16) becomes trivial — it loops over existing summaries instead of re-parsing raw logs.

**Separation of concerns:**
- `session-summarize` → narrative layer (qualitative, cached, human-readable)
- `session-review` → metrics + classification layer (quantitative, computed fresh from the raw log)

The summary is used for **narrative context** in the review's Timeline / Feedback log / Friction sections. **Precise metrics** (time per skill, token counts, verify pass rates, feedback counts) are ALWAYS recomputed from the raw `session-log.jsonl` in Step 1 below — never paraphrased through the summary, because LLM summaries are lossy and would corrupt the numbers.

### Step 1: Load Session Data

```
1. Read session-log.jsonl — parse each line as JSON (raw source for all metrics)
2. Read {key}-summary.md — narrative context produced in Step 0
3. Read metadata.json — get work item context (including revisions[] if present)
4. Read {key}-spec.md — get original acceptance criteria
5. Read {key}-plan.md — get task list and completion markers
6. If the workspace has a revisions/ subfolder, list all sub-folders and read their classification.md files
7. If epic: load all US session-log.jsonl files AND {key}-summary.md files under the epic folder
```

**Rule:** every quantitative metric in steps 2.1–2.8 below MUST be computed from the raw `session-log.jsonl` events. The summary provides narrative color; it never replaces raw data for numbers.
...
*Generated by kitt session-review v2.2 (summary delegated to session-summarize v1.0)*
```
